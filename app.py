...
database_uri = 'mysql+mysqlconnector://estagiario:Celeste123@10.180.134.88:3306/papercontrol'
...
app.config['MAIL_DEFAULT_SENDER'] = 'arthur.bastista@planejamento.mg.gov.br'
    ...
    if request.method == 'POST':
        ...
        if relatorio_type == 'Completo':
            ...
            if tipo_relatorio == 'Completo':
                ...
                usuarios_data = [{'id_user': user.id_user, 'email': user.email,
                                  'tipo_user': user.tipo_user, 'estoque': user.estoque} for user in Usuario.query.all()]
                ...
                if ' to ' in data_param:
                    ...
                    usuarios_df = pd.DataFrame([{'id_user': user.id_user, 'email': user.email,
                                               'tipo_user': user.tipo_user, 'estoque': user.estoque} for user in usuarios_data])
                    ...
                    data_reposicoes = Reposicao.query.filter_by(
            ...
            df = pd.DataFrame([{'id_user': user.id_user, 'email': user.email,
                              'tipo_user': user.tipo_user, 'estoque': user.estoque} for user in data])
...
def ajudaOZe():
    if request.method == 'POST':
        try:
            ...
            usuario = Usuario.query.filter_by(id_user=id_user).first()  # Obter o usuário pelo ID

            if usuario:  # Verificar se o usuário foi encontrado
                # Corrigido para usuario.email
                nova_ajuda = Ajuda(tipo=tipo, descricao=descricao,
                                   id_user=id_user, email=usuario.email)
                db.session.add(nova_ajuda)
                db.session.commit()

                # Envia e-mail
                # Corrigido para usuario.email
                enviar_email(usuario.email, tipo, descricao)
...
def enviar_email(email, tipo, descricao):
    try:
        if email:
            ...
            msg['To'] = email
            msg['Subject'] = f'{tipo.capitalize()} - Solicitação de Ajuda'

            content = f"""
            Olá,

            Aqui estão algumas informações da sua solicitação de ajuda:
            - Tipo: {tipo}
            - Descrição: {descricao}
        ...
        if request.method == 'POST':
            ...
            if file and file.filename.endswith(('.xlsx', '.xls')):
                ...
                resultado_df = pd.DataFrame(columns=[
                                            'prédio', 'andar', 'ilha', 'quantidade_impressa', 'quantidade_reabastecida'])
                ...
                for _, row in df.iterrows():
                    ...
                    reposicoes = Reposicao.query.filter_by(
                        predio=predio, andar=andar, ilha=ilha).all()

                    # Calcular a quantidade total reabastecida
                    quantidade_reabastecida = sum(
                        [r.quantidade_reposicao for r in reposicoes])
